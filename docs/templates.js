...
/* ---- Agent & AI Radar research brief prompt.
   This is not a writer prompt. It turns source facts supplied by the operator
   into a structured learning/content brief, or asks for more evidence. ---- */
window.buildAgentBriefPrompt = function (o) {
  o = o || {};
  var tags = (o.tags || []).join(", ") || "unknown";
  var mode = o.mode || "explain_build";
  var modeNames = {
  ...
  if (!sources.length) {
    if ((o.summary || "").trim()) sources.push({ id:"S1", label:o.sourceLabel || "Collected source excerpt", url:o.source || "", date:o.published || "unknown", text:o.summary });
    if ((o.facts || "").trim()) sources.push({ id:sources.length ? "S2" : "S1", label:"Operator-supplied source text", url:o.source || "", date:o.published || "unknown", text:o.facts });
  }
  var sourcePack = sources.length ? sources.map(function (s, i) {
    var id = s.id || ("S" + (i + 1));
    return "[" + id + "] " + (s.label || "Source material") + "\nURL: " + (s.url || "not supplied") + "\nDATE: " + (s.date || "unknown") + "\nTEXT:\n" + (s.text || "(no source text supplied)");
  }).join("\n\n") : "[S1] Saved reference only\nURL: " + (o.source || "not supplied") + "\nDATE: " + (o.published || "unknown") + "\nTEXT:\n(no source text supplied)";
  var preferences = [
    "Level: " + (o.level || "beginner-friendly with technical depth underneath"),
    "Preferred stack: " + (o.stack || "stack-agnostic"),
    "Constraints: " + (o.constraints || "small, low-cost, manual approval before external side effects")
  ].join("\n");
  var gaps = o.gaps || (!sources.length ? "The URL/title is a reference, not evidence. Ask for an excerpt, README, transcript, documentation, or other source material." : "Identify every material gap yourself; never fill it with a guess.");
  return [
...
(sourcePolicy === "research_enabled"
  ? "You may browse only if tools actually exist. Prefer primary sources, record every retrieved URL and access date as [R1], [R2], disclose inaccessible sources, and keep all new research marked UNREVIEWED."
