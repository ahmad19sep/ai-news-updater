...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  /* reader */
  #reader{position:fixed;inset:0;z-index:80;background:rgba(17,19,22,.5);overflow-y:auto;}
  .rbox{max-width:720px;margin:36px auto;background:#fff;border:1px solid var(--line2);border-radius:6px;padding:0 0 36px;}
  .rclose{position:sticky;top:14px;float:right;margin:14px 14px 0 0;background:#fff;border:1px solid var(--line2);color:var(--ink);border-radius:50%;width:38px;height:38px;font-size:15px;cursor:pointer;z-index:2;}
  .rclose:hover{border-color:var(--ink);}
  .rinner{padding:14px 38px 0;}
  .rbox h1{font-family:var(--serif);font-size:clamp(27px,4vw,42px);line-height:1.08;font-weight:700;letter-spacing:-.015em;margin:12px 0 16px;color:var(--ink);}
  .rmeta{display:flex;align-items:center;gap:11px;padding-bottom:20px;border-bottom:1px solid var(--line);}
  .ravatar{width:40px;height:40px;border-radius:50%;background:var(--ink);display:flex;align-items:center;justify-content:center;font-weight:700;font-size:14px;color:#fff;flex:none;}
  .rauthor{font-size:13.5px;font-weight:700;}
  .rsub{font-size:11.5px;color:var(--dim);}
  .rimg{aspect-ratio:16/9;border-radius:4px;margin:22px 0;border:1px solid var(--line);background-size:cover;background-position:center;}
...
function openArticle(i){
  ...
  document.getElementById("reader").innerHTML='<div class="rbox"><button class="rclose" onclick="closeArticle()">✕</button><div class="rinner">'+
    catTag(p.cat||"Featured",color)+'<h1>'+esc(p.title)+'</h1>'+
    ...
    '<div class="rbody">'+paras+src+'</div>'+
    (rel?'<div class="sec-h" style="margin-top:34px">Related Stories</div><div class="relgrid">'+rel+'</div>':'')+
    '</div></div>';
...
function openPage(key){
  ...
  document.getElementById("reader").innerHTML='<div class="rbox"><button class="rclose" onclick="closeArticle()">✕</button>'+
    '<div class="rinner"><h1>'+p[0]+'</h1><div class="rbody">'+p[1]+'</div></div></div>';
  const rd=document.getElementById("reader"); rd.hidden=false; rd.scrollTop=0; document.body.style.overflow="hidden";
}
