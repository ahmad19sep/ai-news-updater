...
def build_master_prompt(item):
    ...
    return (
        ...
        '[[X_POST]]\n(TEXT-ONLY X post - NO link and NO [ARTICLE LINK] token in it, X downranks link posts; '
        'the link goes in the first reply. Scroll-stopping hook, 3-5 short human lines on what happened + why '
        'it matters and for whom, one line of opinion ("my take:"), FINAL line = a genuine question a stranger '
        'can answer in 5 seconds. Sound like a person reacting, not a news bot)\n'
        ...
        '[[IMAGE_PROMPT]]\n(Imagine a studio of 20 world-class designers, each with their own signature - '
        'Swiss minimalist, tabloid maximalist, magazine editorial, brutalist, data-first, cinematic, '
        'flat-vector, newspaper heritage, collage punk, luxury tech, photojournalist, geometric, retro print, '
        'corporate clean, kinetic, human-first, schematic... - and assign each task to a DIFFERENT designer. '
        'Act like a news ART DIRECTOR: first classify the story (funding / partnership / '
        'launch / policy / people-quote / report / drama), then pick the poster format that FITS it - vary '
        'formats between tasks, never the same layout every time: marker-highlight photo (key phrases on solid '
        'yellow/color bars), lower-third dark band with one accent color, top-headline card (highlight bar '
        'behind opening words), people/quote card (flat color background, cut-out person with white outline, '
        'quote + name + small badge tag), photo caption card (candid photo, bold left caption), category-tag '
        'bold caps (small chip like AI / STARTUPS above ALL-CAPS headline), big-number poster (a huge $28M / '
        '20,000 style number dominates), cutout viral card (cut-out key person centered over dark blurred '
        'background, 1-2 circle inset images of the product, ALL-CAPS condensed headline with the 2-3 key words '
        'in the accent color - best for drama/leaks/CEO stories), breaking strip (red BREAKING tag, full-bleed '
        'photo, thick dark headline band - urgent news), vs/matchup card (split screen, two rivals + big VS '
        'badge - comparisons), or then-vs-now timeline (old year muted left, today vivid right, bold arrow - '
        'progress stories). NEVER the same format two tasks in a row. Then write ONE detailed vertical 4:5 prompt: '
